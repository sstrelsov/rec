#!/usr/bin/env python3
"""
Organize mitmproxy captures into directory structure:
- Each domain gets a directory
- Each API call gets a subdirectory
- request file = httpie command to reproduce the call
- response file = JSON/HTML/etc response with proper extension
"""
...
def organize_api_calls(dump_file, output_dir="api_calls"):
    ...
    # Create the mitmdump script content as a separate file
    script_lines = [
        "import json",
        "import re",
        "from pathlib import Path",
        ...
        "    ",
        "    # Write response file with appropriate extension",
        ...
        "            # Error reading response - save as text with error info",
        "            with open(response_file, 'w') as f:",
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"\n📂 Directory structure:")
            show_tree_structure(output_path)

            print(f"\n💡 Usage:")
            print(f"  cd {output_dir}")
            print(f"  ./api.github.com/get_users_octocat/request      # Run the HTTPie command")
            print(f"  cat api.github.com/get_users_octocat/response.json  # View JSON response")
            print(f"  open static.example.com/get_page/response.html      # Open HTML in browser")
    ...
    try:
        ...
        for i, dir_item in enumerate(dirs[:10]):  # Limit to 10 dirs
            ...
            if current_depth < max_depth - 1:
                show_tree_structure(dir_item, max_depth, current_depth + 1)
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python organize_api_calls.py <dump_file> [output_dir]")
        print()
        print("Examples:")
        print("  python organize_api_calls.py capture.mitm")
        print("  python organize_api_calls.py capture.mitm my_api_collection")
        print()
        print("Creates directory structure:")
        print("  api_calls/")
        print("  ├── api.github.com/")
        print("  │   ├── get_users_octocat/")
        print("  │   │   ├── request           # HTTPie command")
        print("  │   │   └── response.json     # JSON response")
        print("  │   └── get_repos/")
        print("  │       ├── request")
        print("  │       └── response.json")
        print("  ├── static.example.com/")
        print("  │   ├── get_index/")
        print("  │   │   ├── request")
        print("  │   │   └── response.html     # HTML response")
        print("  │   └── get_styles/")
        print("  │       ├── request")
        print("  │       └── response.css      # CSS response")
        print("  └── apollo.io/")
        print("      ├── post_api_v1_events/")
        print("      │   ├── request")
        print("      │   └── response.json")
        return

    dump_file = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) > 2 else "api_calls"

    organize_api_calls(dump_file, output_dir)
